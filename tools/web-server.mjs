/**
 * Slag — a játékot ÉS a szobákat együtt kiszolgáló webkiszolgáló.
 *
 * Az `app.js` indítja (lásd ott, miért olyan rövid az).
 *
 * A cPanel Node-alkalmazását jellemzően a domain gyökerére állítják, és a
 * Passenger onnantól minden kérést az indítófájlnak ad. Ezért itt előbb a
 * relay kap esélyt (`/api/...`), és minden mást a statikus kiszolgáló
 * teljesít a projekt mappájából.
 *
 * Mellékhaszon: a játék és a relay így AZONOS EREDETRŐL jön, tehát semmit
 * nem kell beállítani — a böngésző magától a saját címén keresi a szobákat.
 *
 * Helyben:  node app.js --port 8080
 */
...
import { relayRequest } from './relay-server.mjs';

/** A projekt gyökere: ez a fájl a `tools/` alatt van, tehát egy szinttel feljebb. */
